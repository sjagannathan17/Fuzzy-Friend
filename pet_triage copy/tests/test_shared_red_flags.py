...
def test_check_text_for_red_flags():
    ...
    print(f"  ✓ 'not breathing, collapsed' → CRITICAL")
...
def test_check_text_for_red_flags_breed_specific():
    ...
    print(f"  ✓ Great Dane + bloat symptoms → CRITICAL with breed alert")
...
def test_check_red_flags_combined():
    ...
    assert result["is_er"] == True or result.get("severity") == "CRITICAL"
    ...
    assert result.get("severity") == "CRITICAL" or result.get("is_er") == True
